...
PlayerEyeTrackingExtension.update = function (self, unit, input, dt, context, t)
	...
	self.calc_smooth_gaze_forward(self)
	self.update_gaze_aim(self, dt)
...
PlayerEyeTrackingExtension.reset_aim = function (self)
	...
	local new_target = first_person_extension.current_position(first_person_extension) + Quaternion.forward(first_person_extension.current_rotation(first_person_extension))

	first_person_extension.set_aim_constraint_target(first_person_extension, "tobii_aim", new_target)
	first_person_extension.play_animation_event(first_person_extension, "tobii_aim_inactive")
...
PlayerEyeTrackingExtension.calc_gaze_forward = function (self)
	...
	if not HAS_TOBII or Tobii.user_presence() ~= Tobii.USER_PRESENT or Tobii.device_status() ~= Tobii.DEVICE_TRACKING then
		self.current_gaze_forward:store(Quaternion.forward(first_person_extension.current_rotation(first_person_extension)))
...
PlayerEyeTrackingExtension.calc_smooth_gaze_forward = function (self)
	...
	if not HAS_TOBII or Tobii.user_presence() ~= Tobii.USER_PRESENT or Tobii.device_status() ~= Tobii.DEVICE_TRACKING then
		self.smooth_forward:store(Quaternion.forward(first_person_extension.current_rotation(first_person_extension)))
...
PlayerEyeTrackingExtension.update_gaze_aim = function (self, dt)
	...
	if fire_at_gaze_setting and not self.fire_at_gaze_enabled then
		first_person_extension.play_animation_event(first_person_extension, "tobii_aim_active")
