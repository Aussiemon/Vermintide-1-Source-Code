...
LevelUnitAnimationExtension.hot_join_sync = function (self, sender)
	...
	network_transmit.send_rpc(network_transmit, animation_sync_rpc, sender, level_unit_index, Unit.animation_get_state(unit))
...
LevelUnitAnimationExtension.play_animation_event = function (self, event)
	...
	if network_manager.in_game_session(network_manager) then
		...
		if self.is_server then
			network_transmit.send_rpc_clients(network_transmit, "rpc_level_unit_anim_event", event_id, level_unit_index)
		else
			network_transmit.send_rpc_server(network_transmit, "rpc_level_unit_anim_event", event_id, level_unit_index)
		end
	end
