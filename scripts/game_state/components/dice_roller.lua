...
DiceRoller._request_from_backend = function (self, hero_name)
	...
	ScriptBackendItem.generate_item_server_loot(dice, difficulty, level_start, level_end, hero_name, dlc_name)

	return 
end
DiceRoller._check_for_achievement = function (self, reward_backend_id)
	local hero_trinkets = ScriptBackendItem.get_filtered_items("trinket_as_hero and equipped_by == current_hero")
	...
	if trinket then
		local item = ScriptBackendItem.get_item_from_id(reward_backend_id)
...
DiceRoller.poll_for_backend_result = function (self)
	...
	local successes, win_list, reward_backend_id, level_rewards = ScriptBackendItem.check_for_loot()
