...
ProfileSynchronizer._send_rpc_lobby_clients = function (self, rpc, ...)
	...
	for _, peer_id in ipairs(lobby_members.get_members(lobby_members)) do
...
ProfileSynchronizer.rpc_server_mark_profile_used = function (self, sender, peer_id, local_player_id, previous_profile_index, profile_index)
	...
	if self._is_server then
		self.update_lobby_profile_data(self)
	...
	if peer_id == self._peer_id then
		local inventory_list, inventory_list_first_person = self._inventory_package_synchronizer:build_inventory_lists(profile_index)
...
ProfileSynchronizer.update_lobby_profile_data = function (self)
	local stored_lobby_host_data = self._lobby_host:get_stored_lobby_data()

	if stored_lobby_host_data then
		for i = 1, NUM_PROFILES, 1 do
			local owner_table = self._profile_owners[i]
			local is_human_player = (owner_table and self.is_human_player(self, owner_table.peer_id, owner_table.local_player_id)) or nil
			local unique_id = (is_human_player and self._player_manager:_unique_id(owner_table.peer_id, owner_table.local_player_id)) or nil
			local value = unique_id or "available"
			stored_lobby_host_data["player_slot_" .. i] = value
		end
...
ProfileSynchronizer.resync_loadout = function (self, profile_index, player)
	local inventory_list, inventory_list_first_person = self._inventory_package_synchronizer:build_inventory_lists(profile_index)
	local network_inventory_list = FrameTable.alloc_table()
	local network_inventory_list_first_person = FrameTable.alloc_table()
