...
QuestRewardPresentationUI.destroy = function (self)
	for _, presentation in pairs(self.presentations) do
		presentation.destroy(presentation)
	end
...
QuestRewardPresentationUI.update = function (self, dt, t)
	...
	if self.presentation_done(self) and self.input_manager:any_input_pressed() then
		self.stop_reward_presentation(self)
	end
...
QuestRewardPresentationUI._draw = function (self, dt)
	...
	local input_manager = self.input_manager
	local input_service = input_manager.get_service(input_manager, "quest_reward_presentation_screen_ui")
	local gamepad_active = input_manager.is_device_active(input_manager, "gamepad")

	UIRenderer.begin_pass(ui_renderer, ui_scenegraph, input_service, dt)

	if self.presentation_done(self) then
