...
UnlockKeyView.on_enter = function (self)
	...
	local available_unlock_keys = ScriptBackendProfileAttribute.get("available_unlock_keys")

	if not available_unlock_keys then
		available_unlock_keys = ""

		ScriptBackendProfileAttribute.set("available_unlock_keys", available_unlock_keys)
	end

	local keys = {}

	for key in string.gmatch(available_unlock_keys, "([%w_]+);") do
		keys[#keys + 1] = key
	end

	self.available_unlock_keys = keys
