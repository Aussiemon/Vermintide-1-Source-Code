...
FriendsView.init = function (self, ingame_ui_context)
	...
	input_manager:create_input_service("friends_view", "IngameMenuKeymaps", "IngameMenuFilters")
	input_manager:map_device_to_service("friends_view", "keyboard")
	input_manager:map_device_to_service("friends_view", "mouse")
	input_manager:map_device_to_service("friends_view", "gamepad")
...
FriendsView.set_active = function (self, active)
	self.active = active
	self.selected_id = nil
				...
				if content then
					...
					if id then
						if not self.selected_id then
							...
							self:on_entry_pressed(id, i, true)
						end

						self:setup_widget(friends[id], id, content, item_styles[i])
					end
				end
			end
		end
...
FriendsView.create_ui_elements = function (self)
	...
	self.fade_background = UIWidget.init(widget_definitions.fade_background)
	...
	for i = 1, FRIENDS_LIST_LIMIT, 1 do
		local content, style = self:create_friend_entry(i)
...
FriendsView.cb_refresh_friends_done = function (self, friends_list, use_cached_friends_list)
	...
	for i = 1, playing_bulldozer_friends_n, 1 do
		...
		self:setup_widget(friends[id], id, list_content[index], item_styles[index])
	end
	...
	if selected_id then
		...
		if friend_data then
			self:setup_info_window(friend_data, selected_id)
		end
	end
	...
	if PLATFORM == "ps4" and not use_cached_friends_list then
		self:refresh_psn_room_data(friends, playing_bulldozer_friends)
	end
...
FriendsView.cb_presence_async_done = function (self, friend_data, xuid, data)
	if data.presence then
		...
		local level_name, difficulty, level_image, num_players, presence = self:_extract_level_and_difficulty_from_presence(presence or "")
		...
		self:update_input_description(not disable_invite, false)
	else
		...
		self:update_input_description(false, false)
	end
...
FriendsView.setup_info_window = function (self, friend_data, id, first_update)
	...
	if friend_data.playing_this_game then
		...
		if lobby_data then
			...
			local is_full_group = (num_players and tonumber(num_players) >= 4) or false
	...
	self:update_input_description(not disable_invite, not disable_join)
end
...
FriendsView.on_invite_button_clicked = function (self, gamepad_active)
	...
	if gamepad_active then
		self:start_invite_text_animation()
	end
end
...
FriendsView.on_entry_pressed = function (self, id, index, first_update)
	...
	if PLATFORM == "ps4" then
		self:cb_refresh_friends_done(nil, true)
	else
		self:refresh_friends()
	...
	self:setup_info_window(self.friends[id], id, first_update)
end
...
FriendsView.update = function (self, dt, t, is_sub_menu)
	...
	self:update_invite_cooldowns(dt)
	...
	if self.active then
		...
		local input_service = input_manager:get_service("friends_view")
		local gamepad_active = input_manager:is_device_active("gamepad")

		if not is_sub_menu and (self:on_close_button_clicked() or input_service:get("toggle_menu", true) or input_service:get("back", true)) then
			if gamepad_active then
				self:play_sound("Play_hud_select")
			end
		...
		self:update_scroll()
		self:draw(gamepad_active, dt)
		self:update_buttons(dt)
		self:handle_controller_navigation_input(input_service, dt)
	end
...
FriendsView.draw = function (self, gamepad_active, dt)
	...
	UIRenderer.draw_widget(ui_renderer, self.title)
	...
	if self.selected_id then
		UIRenderer.draw_widget(ui_renderer, self.friend_info_title)
		...
		if self.updating_friend_info then
			...
			UIRenderer.draw_widget(ui_renderer, update_icon_widget)
		else
			UIRenderer.draw_widget(ui_renderer, self.friend_info)

			if not gamepad_active then
				UIRenderer.draw_widget(ui_renderer, self.join_button)
				UIRenderer.draw_widget(ui_renderer, self.invite_button)
			else
				UIRenderer.draw_widget(ui_renderer, self.controller_invite_text)
			end
		end
	end
...
FriendsView.update_buttons = function (self, dt, optional_select_index)
	local friend_list = self.friend_list
	local friend_list_content = friend_list.content.list_content
	...
	for i = start_index, stop_index, 1 do
		...
		if content.button_hotspot.on_release or content.controller_button_hotspot.on_release or optional_select_index == i then
			self:play_sound("Play_hud_select")
			self:on_entry_pressed(content.id, i)
	...
	if self.selected_id then
		...
		if platform ~= "xb1" and not join_button_content.button_hotspot.disabled and (join_button_content.button_hotspot.on_release or input_service:get("special_1")) then
			self:play_sound("Play_hud_select")
			self:on_join_button_clicked()
		elseif not invite_button_content.button_hotspot.disabled and (invite_button_content.button_hotspot.on_release or input_service:get("refresh")) then
			self:play_sound("Play_hud_select")

			invite_button_content.button_hotspot.on_release = nil

			self:on_invite_button_clicked(gamepad_active)
		elseif input_service:get("confirm") then
			self:play_sound("Play_hud_select")
			self:on_show_gamercard_clicked()
		end
	end
end
	...
	if self.controller_cooldown > 0 then
		...
		if new_selection_index and new_selection_index ~= selection_index then
			...
			if content then
				...
				self:play_sound("Play_hud_hover")
				self:on_entry_pressed(id, new_selection_index)

				local is_outside, state = self:is_entry_outside(new_selection_index)
