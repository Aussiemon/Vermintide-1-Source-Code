...
GiftPopupUI._create_ui_elements_level_unlock = function (self, rewards, unlock_settings)
	...
	self:_start_level_presentation(rewards)
...
GiftPopupUI._poll_new_rewards = function (self, dt)
	...
	if poll_cooldown and poll_cooldown > 0 then
		...
		for _, unlock in ipairs(CommunityUnlocks) do
			...
			local title_property_enabled = title_properties:get_value(title_property)
...
GiftPopupUI._set_selection = function (self, index)
	...
	if table.size(reward_units_to_spawn) > 0 then
		...
		if units_loaded then
			...
			self:spawn_link_unit(item_key, offset_z, offset_dir_multiplier)
			self:spawn_reward_units(units_data, first_time, play_reward_sound)
		end
	end
end
