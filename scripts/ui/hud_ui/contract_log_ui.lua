...
ContractLogUI._sync_contract_progression = function (self)
	...
	for _, entry_data in ipairs(log_entries) do
		...
		if self.quest_manager:has_contract_session_changes(contract_id) then
			local changed, task_completed, made_progress, negative_progress = self._update_contract_goal(self, entry_data)
	...
	elseif any_task_made_negative_progress then
		self.play_sound(self, "Play_hud_quest_progress_lost")
	elseif any_task_made_progress then
		self.play_sound(self, "Play_hud_quest_progress")
	end
...
ContractLogUI._update_contract_goal = function (self, entry_data)
	...
	if task then
		...
		if percent_contract then
			...
			if show_bonus and 0 < task_start_progress then
		...
		local _, text_width = self._get_text_size(self, widget_style.task_text, text)
