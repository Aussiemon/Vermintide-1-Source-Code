local Credits = {
	settings = {
		speed = 100
	},
	entries = {
		{
			text = "Fatshark",
			type = "header"
		},
		{
			text = "CEO",
			localized = false,
			type = "title"
		},
		{
			text = "Martin Wahlund",
			type = "person"
		},
		{
			text = "CTO",
			localized = false,
			type = "title"
		},
		{
			text = "Rikard Blomberg",
			type = "person"
		},
		{
			text = "Game Director",
			localized = false,
			type = "title"
		},
		{
			text = "Anders De Geer",
			type = "person"
		},
		{
			text = "Executive Producer",
			localized = false,
			type = "title"
		},
		{
			text = "Mårten Stormdal",
			type = "person"
		},
		{
			text = "Senior Producer",
			localized = false,
			type = "title"
		},
		{
			text = "Robert Bäckström",
			type = "person"
		},
		{
			text = "Producer",
			localized = false,
			type = "title"
		},
		{
			text = "Liam O'Neill",
			type = "person"
		},
		{
			text = "Head of Production",
			localized = false,
			type = "title"
		},
		{
			text = "Erika Kling",
			type = "person"
		},
		{
			text = "Assistant Producer",
			localized = false,
			type = "title"
		},
		{
			text = "Kasper Batalje",
			type = "person"
		},
		{
			text = " ",
			type = "person"
		},
		{
			text = "Design Manager",
			localized = false,
			type = "title"
		},
		{
			text = "Joakim Setterberg",
			type = "person"
		},
		{
			text = "Game Design",
			localized = false,
			type = "title"
		},
		{
			text = "Victor Magnuson",
			type = "person"
		},
		{
			text = "Mats Andersson",
			type = "person"
		},
		{
			text = "Markus Olsén",
			type = "person"
		},
		{
			text = "Kasper Holmberg",
			type = "person"
		},
		{
			text = "Game Writer",
			localized = false,
			type = "title"
		},
		{
			text = "Magnus Liljedahl",
			type = "person"
		},
		{
			text = "Story and Dialogue",
			localized = false,
			type = "title"
		},
		{
			text = "Matthew Ward",
			type = "person"
		},
		{
			text = "Andy Hall",
			type = "person"
		},
		{
			text = "Lead Level Designer",
			localized = false,
			type = "title"
		},
		{
			text = "Daniel Platt",
			type = "person"
		},
		{
			text = "Level Design",
			localized = false,
			type = "title"
		},
		{
			text = "Adam Timén",
			type = "person"
		},
		{
			text = "Jennika Cederholm",
			type = "person"
		},
		{
			text = "Sara Sällemark",
			type = "person"
		},
		{
			text = " ",
			type = "person"
		},
		{
			text = "Lead Programmer",
			localized = false,
			type = "title"
		},
		{
			text = "Joakim Wahlström",
			type = "person"
		},
		{
			text = "Programming Manager",
			localized = false,
			type = "title"
		},
		{
			text = "Peter Nilsson",
			type = "person"
		},
		{
			text = "Technical Director",
			localized = false,
			type = "title"
		},
		{
			text = "Robin Hagblom",
			type = "person"
		},
		{
			text = "CIO",
			localized = false,
			type = "title"
		},
		{
			text = "Johan Jonker",
			type = "person"
		},
		{
			text = "Principal Platform Programmers",
			localized = false,
			type = "title"
		},
		{
			text = "Patrik Wennersten",
			type = "person"
		},
		{
			text = "Staffan Tejre",
			type = "person"
		},
		{
			text = "Programming",
			localized = false,
			type = "title"
		},
		{
			text = "Axel Kinner",
			type = "person"
		},
		{
			text = "Tom Batsford",
			type = "person"
		},
		{
			text = "Dmytro Vovk",
			type = "person"
		},
		{
			text = "Elias Stolt",
			type = "person"
		},
		{
			text = "Sebastian Granstrand",
			type = "person"
		},
		{
			text = "Christoffer Wiss",
			type = "person"
		},
		{
			text = "Adam Skoglund",
			type = "person"
		},
		{
			text = "Peder Nordenström",
			type = "person"
		},
		{
			text = "Niklas Häll",
			type = "person"
		},
		{
			text = "Karl Werf",
			type = "person"
		},
		{
			text = "Philip Sköld",
			type = "person"
		},
		{
			text = "Additional Programming",
			localized = false,
			type = "title"
		},
		{
			text = "Olle Grahn",
			type = "person"
		},
		{
			text = "Mathias Södermark",
			type = "person"
		},
		{
			text = "Branimir Sokolov",
			type = "person"
		},
		{
			text = "Egil Tordengren",
			type = "person"
		},
		{
			text = "Daniel Magnusson",
			type = "person"
		},
		{
			text = "Axel Lewenhaupt",
			type = "person"
		},
		{
			text = " ",
			type = "person"
		},
		{
			text = "Environment Art Manager",
			localized = false,
			type = "title"
		},
		{
			text = "Arvid Nilsson",
			type = "person"
		},
		{
			text = "Lead Environment Artist",
			localized = false,
			type = "title"
		},
		{
			text = "Robert Berg",
			type = "person"
		},
		{
			text = "Environment Art",
			localized = false,
			type = "title"
		},
		{
			text = "Niklas Eneqvist",
			type = "person"
		},
		{
			text = "Robin Lundin",
			type = "person"
		},
		{
			text = "Simon Jonasson",
			type = "person"
		},
		{
			text = "Character Art & Outsourcing Manager",
			localized = false,
			type = "title"
		},
		{
			text = "Johan Lorentzen",
			type = "person"
		},
		{
			text = "Corpses Critter Statues Artist",
			localized = false,
			type = "title"
		},
		{
			text = "Gabriel Forsén",
			type = "person"
		},
		{
			text = "Concept Art",
			localized = false,
			type = "title"
		},
		{
			text = "Niklas Frostgård",
			type = "person"
		},
		{
			text = "Mathias Sällström",
			type = "person"
		},
		{
			text = "Mattias Rousk",
			type = "person"
		},
		{
			text = "Petter Lundh",
			type = "person"
		},
		{
			text = "Jonas Norlén",
			type = "person"
		},
		{
			text = "Patrik Rosander",
			type = "person"
		},
		{
			text = "Lead Animator & TA",
			localized = false,
			type = "title"
		},
		{
			text = "Mikael Hansson",
			type = "person"
		},
		{
			text = "Erik Lindqvist",
			type = "person"
		},
		{
			text = "Animation",
			localized = false,
			type = "title"
		},
		{
			text = "Madeleine Von Post",
			type = "person"
		},
		{
			text = "Elin Mikkelsen",
			type = "person"
		},
		{
			text = "Patrik Ånberg",
			type = "person"
		},
		{
			text = "David Nilsson",
			type = "person"
		},
		{
			text = "VFX",
			localized = false,
			type = "title"
		},
		{
			text = "Isak Bergh",
			type = "person"
		},
		{
			text = "Level Art",
			localized = false,
			type = "title"
		},
		{
			text = "Tomas Holm",
			type = "person"
		},
		{
			text = " ",
			type = "person"
		},
		{
			text = "Lead Sound Designer",
			localized = false,
			type = "title"
		},
		{
			text = "David Wahlund",
			type = "person"
		},
		{
			text = "Sound Design",
			localized = false,
			type = "title"
		},
		{
			text = "Jonas Hellberg",
			type = "person"
		},
		{
			text = "Danijel Djuric",
			type = "person"
		},
		{
			text = "Technical VO Designer",
			localized = false,
			type = "title"
		},
		{
			text = "Johan Vargek",
			type = "person"
		},
		{
			text = "Audio Engineer",
			localized = false,
			type = "title"
		},
		{
			text = "Linus Söderlund",
			type = "person"
		},
		{
			text = "Music",
			localized = false,
			type = "title"
		},
		{
			text = "Jesper Kyd",
			type = "person"
		},
		{
			text = "Solo Performers",
			localized = false,
			type = "title"
		},
		{
			text = "Cameron Stone",
			type = "person"
		},
		{
			text = "Diego Stocco",
			type = "person"
		},
		{
			text = "Roger Neill",
			type = "person"
		},
		{
			text = " ",
			type = "person"
		},
		{
			text = "COO & Head of Marketing",
			localized = false,
			type = "title"
		},
		{
			text = "Sven Folkesson",
			type = "person"
		},
		{
			text = "Marketing & PR Coordinator",
			localized = false,
			type = "title"
		},
		{
			text = "Cecilia Larsson",
			type = "person"
		},
		{
			text = "Marketing & PR Advisor",
			localized = false,
			type = "title"
		},
		{
			text = "Gunnar Johansson",
			type = "person"
		},
		{
			text = "Community Manager",
			localized = false,
			type = "title"
		},
		{
			text = "Leo Wakelin",
			type = "person"
		},
		{
			text = "Head Of Administration",
			localized = false,
			type = "title"
		},
		{
			text = "Martin Karlsson",
			type = "person"
		},
		{
			text = "IT Technician",
			localized = false,
			type = "title"
		},
		{
			text = "Niklas Johansson",
			type = "person"
		},
		{
			text = "Additional IT Technician",
			localized = false,
			type = "title"
		},
		{
			text = "Frank Hammar",
			type = "person"
		},
		{
			text = " ",
			type = "person"
		},
		{
			text = "QA Lead",
			localized = false,
			type = "title"
		},
		{
			text = "Otto Elggren",
			type = "person"
		},
		{
			text = " ",
			type = "person"
		},
		{
			text = "Quality Assurance",
			localized = false,
			type = "title"
		},
		{
			text = "Emilia Johansson Kiviaho",
			type = "person"
		},
		{
			text = "Rasmus Liljenberg",
			type = "person"
		},
		{
			text = "Viktor Dahlberg",
			type = "person"
		},
		{
			text = "Rickard Sjögren",
			type = "person"
		},
		{
			text = "Eric Sernfalk",
			type = "person"
		},
		{
			text = "Philip Johansson",
			type = "person"
		},
		{
			text = " ",
			type = "person"
		},
		{
			text = "Voice Actors",
			localized = false,
			type = "header"
		},
		{
			text = "credits_witch_hunter",
			localized = true,
			type = "title"
		},
		{
			text = "Tim Bentinck",
			type = "person"
		},
		{
			text = "credits_bright_wizard",
			localized = true,
			type = "title"
		},
		{
			text = "Bethan Dickson Bate",
			type = "person"
		},
		{
			text = "credits_sergeant",
			localized = true,
			type = "title"
		},
		{
			text = "Dan Mersh",
			type = "person"
		},
		{
			text = "credits_waywatcher",
			localized = true,
			type = "title"
		},
		{
			text = "Alix Wilton Regan",
			type = "person"
		},
		{
			text = "credits_dwarf_ranger",
			localized = true,
			type = "title"
		},
		{
			text = "David Rintoul",
			type = "person"
		},
		{
			text = "credits_innkeeper",
			localized = true,
			type = "title"
		},
		{
			text = "David Shaw Parker",
			type = "person"
		},
		{
			text = "credits_ferry_lady",
			localized = true,
			type = "title"
		},
		{
			text = "Nicolette McKenzie",
			type = "person"
		},
		{
			text = "credits_rasknitt",
			localized = true,
			type = "title"
		},
		{
			text = "Andreas Rylander",
			type = "person"
		},
		{
			text = "credits_skaven",
			localized = true,
			type = "title"
		},
		{
			text = "James Hogg",
			type = "person"
		},
		{
			text = "Indy Neidell",
			type = "person"
		},
		{
			text = " ",
			type = "person"
		},
		{
			text = "Fatshark Board of Directors - Non Executive",
			localized = false,
			type = "header"
		},
		{
			text = "Thomas Lindgren, Chairman of The Board",
			type = "person"
		},
		{
			text = "Stefan Lindeberg",
			type = "person"
		},
		{
			text = "Stina Vällfors",
			type = "person"
		},
		{
			text = "Pixeldiet",
			localized = false,
			type = "header"
		},
		{
			text = "Programming",
			localized = false,
			type = "title"
		},
		{
			text = "Anders Elfgren",
			type = "person"
		},
		{
			text = "Fredrik Engkvist",
			type = "person"
		},
		{
			text = "Simon Lundmark",
			type = "person"
		},
		{
			text = " ",
			type = "person"
		},
		{
			text = "Nordic Games GmbH",
			localized = false,
			type = "header"
		},
		{
			text = "Production Team:",
			localized = false,
			type = "title"
		},
		{
			text = "Reinhard Pollice",
			type = "person"
		},
		{
			text = "Roger Joswig",
			type = "person"
		},
		{
			text = "Gennaro Giani",
			type = "person"
		},
		{
			text = "Martin Kreuch",
			type = "person"
		},
		{
			text = "Localization Manager:",
			localized = false,
			type = "title"
		},
		{
			text = "Gennaro Giani",
			type = "person"
		},
		{
			text = "PR & Marketing Director:",
			localized = false,
			type = "title"
		},
		{
			text = "Philipp Brock",
			type = "person"
		},
		{
			text = "PR & Marketing",
			localized = false,
			type = "title"
		},
		{
			text = "Stephanie Harman",
			type = "person"
		},
		{
			text = "Social Media Manager:",
			localized = false,
			type = "title"
		},
		{
			text = "Manuel Karl",
			type = "person"
		},
		{
			text = "Lead Graphic Artist:",
			localized = false,
			type = "title"
		},
		{
			text = "Ernst Satzinger",
			type = "person"
		},
		{
			text = "Graphic Asset Assistant:",
			localized = false,
			type = "title"
		},
		{
			text = "Tobias Grimus",
			type = "person"
		},
		{
			text = "Peter Hambsch",
			type = "person"
		},
		{
			text = "Age Rating Coordinator:",
			localized = false,
			type = "title"
		},
		{
			text = "Thomas Reisinger",
			type = "person"
		},
		{
			text = "Web Developer:",
			localized = false,
			type = "title"
		},
		{
			text = "Nina Trabe",
			type = "person"
		},
		{
			text = "Sales Director:",
			localized = false,
			type = "title"
		},
		{
			text = "Georg Klotzberg",
			type = "person"
		},
		{
			text = "Sales:",
			localized = false,
			type = "title"
		},
		{
			text = "Reinhold Schor",
			type = "person"
		},
		{
			text = "Nik Blower",
			type = "person"
		},
		{
			text = "Ian Warley",
			type = "person"
		},
		{
			text = "Digital Distribution:",
			localized = false,
			type = "title"
		},
		{
			text = "Thomas Reisinger",
			type = "person"
		},
		{
			text = "Tim Grainer",
			type = "person"
		},
		{
			text = "Manufacturing:",
			localized = false,
			type = "title"
		},
		{
			text = "Anton Seicarescu",
			type = "person"
		},
		{
			text = "Accounting & Office Management:",
			localized = false,
			type = "title"
		},
		{
			text = "Marion Mayer",
			type = "person"
		},
		{
			text = "Anton Seicarescu",
			type = "person"
		},
		{
			text = "Business & Product Development Director:",
			localized = false,
			type = "title"
		},
		{
			text = "Reinhard Pollice",
			type = "person"
		},
		{
			text = " ",
			type = "person"
		},
		{
			text = "Nordic Games GmbH Management:",
			localized = false,
			type = "title"
		},
		{
			text = "Klemens Kreuzer",
			type = "person"
		},
		{
			text = "Lars Wingefors",
			type = "person"
		},
		{
			text = " ",
			type = "person"
		},
		{
			text = "Nordic Games NA Inc.",
			localized = false,
			type = "title"
		},
		{
			text = "Adrienne Lauer",
			type = "person"
		},
		{
			text = "Eric Wuestmann",
			type = "person"
		},
		{
			text = "Klemens Kreuzer",
			type = "person"
		},
		{
			text = " ",
			type = "person"
		},
		{
			text = "Plan of Attack (External PR Agency)",
			localized = false,
			type = "header"
		},
		{
			text = "Chris Clarke ",
			type = "person"
		},
		{
			text = "Derick Thomas",
			type = "person"
		},
		{
			text = "Tricia Gray ",
			type = "person"
		},
		{
			text = "Aidan Minter",
			type = "person"
		},
		{
			text = "Tom Davis",
			type = "person"
		},
		{
			text = "Audra McIver",
			type = "person"
		},
		{
			text = "Laura Pauzolyte",
			type = "person"
		},
		{
			text = "QLOC S.A.",
			localized = false,
			type = "header"
		},
		{
			text = "General Manager",
			localized = false,
			type = "title"
		},
		{
			text = "Adam Piesiak",
			type = "person"
		},
		{
			text = "Business Development Director",
			localized = false,
			type = "title"
		},
		{
			text = "Pawel Grzywaczewski",
			type = "person"
		},
		{
			text = "Director of Account Management",
			localized = false,
			type = "title"
		},
		{
			text = "Pawel Ziajka",
			type = "person"
		},
		{
			text = "Account Managers",
			localized = false,
			type = "title"
		},
		{
			text = "Marta Olejniczak",
			type = "person"
		},
		{
			text = "Jakub Trudzik",
			type = "person"
		},
		{
			text = "Head of Quality Assurance",
			localized = false,
			type = "title"
		},
		{
			text = "Sergiusz Slosarczyk",
			type = "person"
		},
		{
			text = "QA Project Managers",
			localized = false,
			type = "title"
		},
		{
			text = "Bartosz Antecki",
			type = "person"
		},
		{
			text = "QA Lab Managers",
			localized = false,
			type = "title"
		},
		{
			text = "Lukasz Miroslawski",
			type = "person"
		},
		{
			text = "Pawel Strzelczyk",
			type = "person"
		},
		{
			text = "QA Team Leader",
			localized = false,
			type = "title"
		},
		{
			text = "Konrad Kolacki",
			type = "person"
		},
		{
			text = "QA Compliance Engineers",
			localized = false,
			type = "title"
		},
		{
			text = "Ewa Angielska",
			localized = false,
			type = "person"
		},
		{
			text = "Anna Bartosik",
			localized = false,
			type = "person"
		},
		{
			text = "Rafal Dabrowski",
			localized = false,
			type = "person"
		},
		{
			text = "Pawel Jaskolski",
			localized = false,
			type = "person"
		},
		{
			text = "Maksym Sapetto",
			localized = false,
			type = "person"
		},
		{
			text = "Kamil Zielinski",
			localized = false,
			type = "person"
		},
		{
			text = "QA Testers",
			localized = false,
			type = "title"
		},
		{
			text = "Sebastian Jaskolka",
			localized = false,
			type = "person"
		},
		{
			text = "Marcin Kilinski",
			localized = false,
			type = "person"
		},
		{
			text = "Dominika Kowalska",
			localized = false,
			type = "person"
		},
		{
			text = "Lukasz Mazur",
			localized = false,
			type = "person"
		},
		{
			text = "Marek Malagocki",
			localized = false,
			type = "person"
		},
		{
			text = "Piotr Milewski",
			localized = false,
			type = "person"
		},
		{
			text = "Jacek Zbikowski",
			localized = false,
			type = "person"
		},
		{
			text = "Localization Testers",
			localized = false,
			type = "title"
		},
		{
			text = "Henryk Borzymowski",
			localized = false,
			type = "person"
		},
		{
			text = "Ismael Garcia-Marlowe",
			localized = false,
			type = "person"
		},
		{
			text = "Laetitia Magniez",
			localized = false,
			type = "person"
		},
		{
			text = "Angela Pellegrino",
			localized = false,
			type = "person"
		},
		{
			text = "Polina Rutkowska",
			localized = false,
			type = "person"
		},
		{
			text = "Maksym Sapetto",
			localized = false,
			type = "person"
		},
		{
			text = "Pablo Venzke Avila",
			localized = false,
			type = "person"
		},
		{
			text = "IT Manager",
			localized = false,
			type = "title"
		},
		{
			text = "Tomasz Dziedzic",
			type = "person"
		},
		{
			text = " ",
			type = "person"
		},
		{
			text = "OMUK London",
			localized = false,
			type = "header"
		},
		{
			text = "Director",
			localized = false,
			type = "title"
		},
		{
			text = "Mark Estdale",
			type = "person"
		},
		{
			text = "Casting",
			localized = false,
			type = "title"
		},
		{
			text = "OMUK",
			type = "person"
		},
		{
			text = "IT Manager",
			localized = false,
			type = "title"
		},
		{
			text = "Juan Manuel Delfin",
			type = "person"
		},
		{
			text = "Victoria Prentice",
			type = "person"
		},
		{
			text = "Production Assistant",
			localized = false,
			type = "title"
		},
		{
			text = "Ben Maltz-Jones",
			type = "person"
		},
		{
			text = "Recording Engineers",
			localized = false,
			type = "title"
		},
		{
			text = "Juan Manuel Delfin",
			type = "person"
		},
		{
			text = "Victoria Prentice",
			type = "person"
		},
		{
			text = "Dialogue Editors",
			localized = false,
			type = "title"
		},
		{
			text = "Lewis Bean",
			type = "person"
		},
		{
			text = "Matt Panayiotopoulos",
			type = "person"
		},
		{
			text = "Marta Puerto",
			type = "person"
		},
		{
			text = "Michael Redhead",
			type = "person"
		},
		{
			text = " ",
			type = "person"
		},
		{
			text = "Imagination Studios",
			type = "header"
		},
		{
			text = "Studio Director",
			type = "title"
		},
		{
			text = "Anton Söderhäll",
			type = "person"
		},
		{
			text = "Associate Producers",
			type = "title"
		},
		{
			text = "Peter Levin",
			type = "person"
		},
		{
			text = "Annika Torell Österman",
			type = "person"
		},
		{
			text = "Animation Manager",
			type = "title"
		},
		{
			text = "Andrew Hutchinson",
			type = "person"
		},
		{
			text = "Motion Capture Supervisor",
			type = "title"
		},
		{
			text = "Samuel Tyskling",
			type = "person"
		},
		{
			text = "Motion Capture Lead/Stage Manager",
			type = "title"
		},
		{
			text = "David Grice",
			type = "person"
		},
		{
			text = "Software Developer",
			type = "title"
		},
		{
			text = "Jacob Alenius",
			type = "person"
		},
		{
			text = "Motion Capture Artists",
			type = "title"
		},
		{
			text = "Nils Aulin",
			type = "person"
		},
		{
			text = "Johan Melander",
			type = "person"
		},
		{
			text = "Motion Capture Actors",
			type = "title"
		},
		{
			text = "Seth Ericson - Svenska Stuntgruppen (Skaven, Sienna Fuegonasus)",
			type = "person"
		},
		{
			text = "Nicklas Hansson - Svenska Stuntgruppen (Skaven, Markus Kruber, Victor Saltzpyre)",
			type = "person"
		},
		{
			text = "Tim Man - Svenska Stuntgruppen (Skaven, Kerillian, Bardin Goreksson)",
			type = "person"
		},
		{
			text = "Tove Vahlne (Kerillian, Sienna Fuegonasus)",
			type = "person"
		},
		{
			text = "Philip Hughes (Bardin Goreksson, Markus Kruber)",
			type = "person"
		},
		{
			text = "Bläck",
			localized = false,
			type = "header"
		},
		{
			text = "Executive Producers",
			localized = false,
			type = "title"
		},
		{
			text = "Tom Olsson",
			type = "person"
		},
		{
			text = "Peter Levin",
			type = "person"
		},
		{
			text = "Annika Torell Österman",
			type = "person"
		},
		{
			text = "Director",
			localized = false,
			type = "title"
		},
		{
			text = "Fredrik Löfberg",
			type = "person"
		},
		{
			text = "Creative Director",
			localized = false,
			type = "title"
		},
		{
			text = "Gustaf Holmsten",
			type = "person"
		},
		{
			text = "VFX Supervisor",
			localized = false,
			type = "title"
		},
		{
			text = "Henrik Eklundh",
			type = "person"
		},
		{
			text = "Lead Character Artist",
			localized = false,
			type = "title"
		},
		{
			text = "Jonas Skoog",
			type = "person"
		},
		{
			text = "Lead Animator",
			localized = false,
			type = "title"
		},
		{
			text = "Jonas Ekman",
			type = "person"
		},
		{
			text = "Light and FX TD",
			localized = false,
			type = "title"
		},
		{
			text = "Simon Rainerson",
			type = "person"
		},
		{
			text = "Lead Compositor",
			localized = false,
			type = "title"
		},
		{
			text = "Peter Blomstrand",
			type = "person"
		},
		{
			text = "Mattepaint and Compositor",
			localized = false,
			type = "title"
		},
		{
			text = "Calle Granström",
			type = "person"
		},
		{
			text = "Production Manager",
			localized = false,
			type = "title"
		},
		{
			text = "Pontus Garmvild",
			type = "person"
		},
		{
			text = "Rigging TD",
			localized = false,
			type = "title"
		},
		{
			text = "Peter Jemstedt",
			type = "person"
		},
		{
			text = "Production Assistant",
			localized = false,
			type = "title"
		},
		{
			text = "Jonathan Forefält",
			type = "person"
		},
		{
			text = "Animator",
			localized = false,
			type = "title"
		},
		{
			text = "Janak Thakker",
			type = "person"
		},
		{
			text = "Junior 3D Artist",
			localized = false,
			type = "title"
		},
		{
			text = "Sebastian Salvo",
			type = "person"
		},
		{
			text = "Junior 3D Artist",
			localized = false,
			type = "title"
		},
		{
			text = "Erik Tylberg",
			type = "person"
		},
		{
			text = "Junior Animator",
			localized = false,
			type = "title"
		},
		{
			text = "Jonas Schild",
			type = "person"
		},
		{
			text = "Concept Artist",
			localized = false,
			type = "title"
		},
		{
			text = "Andree Wallin",
			type = "person"
		},
		{
			text = "Character Artist",
			localized = false,
			type = "title"
		},
		{
			text = "Daniel Bystedt",
			type = "person"
		},
		{
			text = " ",
			type = "person"
		},
		{
			text = "Playtesters",
			localized = false,
			type = "header"
		},
		{
			text = "Jonathan Davidsson",
			type = "person"
		},
		{
			text = "Marcus Davidsson",
			type = "person"
		},
		{
			text = "Jonatan Skoogh",
			type = "person"
		},
		{
			text = "Christian Eriksson",
			type = "person"
		},
		{
			text = "Jonathan Gard",
			type = "person"
		},
		{
			text = "Peter Jemstedt",
			type = "person"
		},
		{
			text = "Andreas Jemstedt",
			type = "person"
		},
		{
			text = "Pontus Rundqvist",
			type = "person"
		},
		{
			text = "Andreas Valter",
			type = "person"
		},
		{
			text = "Johan Norén",
			type = "person"
		},
		{
			text = "Rasmus Karlsson",
			type = "person"
		},
		{
			text = "Annika Fogelgren",
			type = "person"
		},
		{
			text = "Axel Borg",
			type = "person"
		},
		{
			text = "Gustav Borg",
			type = "person"
		},
		{
			text = "Niklas Sylvén",
			type = "person"
		},
		{
			text = "Johan Sylvén",
			type = "person"
		},
		{
			text = "William Löfgren",
			type = "person"
		},
		{
			text = "Markus Olsen",
			type = "person"
		},
		{
			text = "Chris Nordgren",
			type = "person"
		},
		{
			text = "Linus Hombre",
			type = "person"
		},
		{
			text = "Christoffer Ågren",
			type = "person"
		},
		{
			text = "Jochum Skoglund",
			type = "person"
		},
		{
			text = "Oskar Törnros",
			type = "person"
		},
		{
			text = "Fredrik Carlbark",
			type = "person"
		},
		{
			text = "Eric Carlbark",
			type = "person"
		},
		{
			text = "Anders Jansson",
			type = "person"
		},
		{
			text = "Ernst De Geer",
			type = "person"
		},
		{
			text = "Niklas Sylvén",
			type = "person"
		},
		{
			text = "Johan Sylvén",
			type = "person"
		},
		{
			text = "Anton Agerberg",
			type = "person"
		},
		{
			text = "Daniel Stormdal",
			type = "person"
		},
		{
			text = "Björn Qvarfordt",
			type = "person"
		},
		{
			text = "Andreas Wictorin",
			type = "person"
		},
		{
			text = "Aeronwen Trewent",
			type = "person"
		},
		{
			text = "Camilla Olsson",
			type = "person"
		},
		{
			text = "Jonas Linderoth",
			type = "person"
		},
		{
			text = "Daniel Wiksten",
			type = "person"
		},
		{
			text = "Leif Westerholm",
			type = "person"
		},
		{
			text = "Torbjörn Eriksson",
			type = "person"
		},
		{
			text = "Linda Kiby",
			type = "person"
		},
		{
			text = "Johan Nyberg",
			type = "person"
		},
		{
			text = " ",
			type = "person"
		},
		{
			text = "adsk_legal_1",
			localized = true,
			type = "legal"
		},
		{
			text = " ",
			type = "legal"
		},
		{
			text = "fatshark_legal_1",
			localized = true,
			type = "legal"
		},
		{
			text = " ",
			type = "legal"
		},
		{
			text = "gw_legal_credits",
			localized = true,
			type = "legal"
		},
		{
			text = "gw_legal_credits_2",
			localized = true,
			type = "legal"
		},
		{
			text = "gw_legal_credits_3",
			localized = true,
			type = "legal"
		},
		{
			text = "gw_legal_credits_4",
			localized = true,
			type = "legal"
		},
		{
			text = "gw_legal_credits_5",
			localized = true,
			type = "legal"
		},
		{
			text = "gw_legal_credits_6",
			localized = true,
			type = "legal"
		},
		{
			text = "gw_legal_credits_7",
			localized = true,
			type = "legal"
		},
		{
			text = " ",
			type = "legal"
		},
		{
			text = "partner_legal_1",
			localized = true,
			type = "legal"
		},
		{
			text = " ",
			type = "legal"
		},
		{
			text = "partner_legal_2_1",
			localized = true,
			type = "legal"
		},
		{
			text = "partner_legal_2_2",
			localized = true,
			type = "legal"
		},
		{
			text = " ",
			type = "legal"
		},
		{
			text = "partner_legal_3",
			localized = true,
			type = "legal"
		},
		{
			text = " ",
			type = "legal"
		},
		{
			text = "partner_legal_4",
			localized = true,
			type = "legal"
		},
		{
			text = " ",
			type = "person"
		},
		{
			text = "credits_special_thanks_to",
			localized = true,
			type = "header"
		},
		{
			text = "Mikail Yazbeck",
			type = "person"
		},
		{
			text = "Martin Tittenberger, MrT",
			type = "person"
		},
		{
			text = "Sampo Lappalainen",
			type = "person"
		},
		{
			text = "Jasin Bushnaief",
			type = "person"
		},
		{
			text = "Mike Pall, LuaJIT",
			type = "person"
		},
		{
			text = "Robin Flodin, Toadman",
			type = "person"
		},
		{
			text = "Nathaniel Blue, Valve",
			type = "person"
		},
		{
			text = "Ulf Andersson, 10 Chambers",
			type = "person"
		},
		{
			text = "Jonas Lundberg, Early Investor and Financial Advisor",
			type = "person"
		},
		{
			text = "Richard Hough Phoenix Labs",
			type = "person"
		},
		{
			text = "Sean Bender Phoenix Labs",
			type = "person"
		},
		{
			text = "Tobias Barth, Web design",
			type = "person"
		},
		{
			text = "Sara Gustafsson, SHB",
			type = "person"
		},
		{
			text = "Joel Björkland, QA help and lovely hand drawn maps",
			type = "person"
		},
		{
			text = "Kostas Havlatzopoulos Public Clean",
			type = "person"
		},
		{
			text = "Gustaf Ström Partner at DLA Nordic",
			type = "person"
		},
		{
			text = "Richard Folke Partner at DLA Nordic",
			type = "person"
		},
		{
			text = "Nicole De Bernardi for assisting with Italian localization",
			type = "person"
		},
		{
			text = "Rockelstad Slott, Making Christmas happen at Fatshark",
			type = "person"
		},
		{
			text = "Autodesk Mergers and Acqustions, Undisclosed",
			type = "person"
		},
		{
			text = "Codename Starlight, Undisclosed",
			type = "person"
		},
		{
			text = "All our families and friends for their patience",
			type = "person"
		},
		{
			text = "All our great fans and beta testers",
			type = "person"
		},
		{
			text = "&",
			type = "person"
		},
		{
			text = "Games Workshop",
			type = "person"
		},
		{
			text = " ",
			type = "person"
		},
		{
			text = " ",
			type = "person"
		},
		{
			text = " ",
			type = "person"
		},
		{
			text = " ",
			type = "person"
		},
		{
			text = " ",
			type = "person"
		},
		{
			text = " ",
			type = "person"
		},
		{
			text = "...and John Blanche, for unsurpassed inspiration.",
			type = "person"
		}
	}
}

return Credits
