...
InputService.get_filter_keymapping = function (self, keymap_name, optional_platform)
	local filters = self:get_active_filters(optional_platform)
	...
	if type(mappings) == "table" then
		for _, input in pairs(mappings) do
			local key = self:get_keymapping(input, optional_platform)
		...
		return self:get_keymapping(mappings, optional_platform)
	end
end
