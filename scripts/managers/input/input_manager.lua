	...
	for name, keymap in pairs(new_keymaps) do
		...
		for j = 1, n_keymap, 3 do
			...
			if self.platform == "ps4" or self.platform == "xb1" then
				if keymap[j + 2] == "axis" then
					key_index = input_device.axis_index(keymap[j + 1])
				else
					key_index = input_device.button_index(keymap[j + 1])
				end

				if not key_index then
					printf("No such %q %q in input device type %q.", tostring(keymap[j + 2]), keymap[j + 1], input_device_type)
				end
			elseif keymap[j + 2] == "axis" then
				key_index = input_device.axis_index(keymap[j + 1])

				assert(key_index, "No such axis %q in input device type %q.", keymap[j + 1], input_device_type)
			else
				key_index = input_device.button_index(keymap[j + 1])

				assert(key_index, "No such key %q in input device type %q.", keymap[j + 1], input_device_type)
			end
