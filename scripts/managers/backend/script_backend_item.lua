...
Items.poll_upgrades = function (self)
	...
	if upgrades_item and upgrades_item:is_done() and not upgrades_item:error_message() then
...
backend_items = backend_items or Items:new()
...
ScriptBackendItem.forge = function (items, num_tokens, token_type)
	...
	local queue = backend_items:data_server_queue()
	local item = queue:add_item(fuse_script, "param_items_to_merge", items_to_merge, "params_num_tokens", num_tokens, "params_token_type", token_type)

	item:disable_registered_commands()
