...
VoteManager.request_vote = function (self, name, vote_data, voter_peer_id)
	...
	if self.is_server then
		...
		if start_new_voting then
			self.server_start_vote(self, name, nil, vote_data)
...
VoteManager.vote_result = function (self, vote_time_ended)
	...
	local num_of_votes, current_vote_results = self.number_of_votes(self)
...
VoteManager.server_start_vote = function (self, name, ignore_peer_list, data)
	...
	self.active_voting = {
		...
		votes = {},
		voters = self.get_voter_start_list(self, ignore_peer_list),
		data = data
	}
...
VoteManager.rpc_vote = function (self, peer_id, vote_cast)
	...
	if active_voting then
		...
		self.server_add_vote(self, peer_id, vote_cast)
	end
...
VoteManager.server_update = function (self, dt, t)
	...
	active_voting.voters = self.update_voter_list_by_active_peers(self, active_voting.voters)
	local vote_time_ended = self.time_ended(self, t)
	local vote_result = self.vote_result(self, vote_time_ended)
...
VoteManager.rpc_client_start_vote_peer_id = function (self, peer_id, vote_type_id, sync_data)
	self.start_vote_base(self, peer_id, vote_type_id, sync_data)
...
VoteManager.rpc_client_complete_vote = function (self, sender_id, vote_result)
	if self.active_voting then
		local number_of_votes, vote_results = self.number_of_votes(self)
