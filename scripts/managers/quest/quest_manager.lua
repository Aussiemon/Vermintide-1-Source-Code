...
QuestManager.init = function (self, statistics_db, level_key)
	...
	if backend_settings.quests_enabled then
		...
		quest_interface:are_contracts_dirty()
		quest_interface:are_quests_dirty()
		quest_interface:are_expire_times_dirty()

		self.quest_interface = quest_interface
		self.session_progress = {}
		self.all_quests = quest_interface:get_quests()
		self.active_quest = quest_interface:get_active_quest()
		self.available_quests = quest_interface:get_available_quests()
		self.all_contracts = quest_interface:get_contracts()
		self.active_contracts = quest_interface:get_active_contracts()
		self.available_contracts = quest_interface:get_available_contracts()
		self.mutators = quest_interface:get_mutators()
		self.status = quest_interface:get_status()
		self.active_mutators = self:_update_active_mutators({})
	end
end
...
QuestManager.evaluate_level_end = function (self, statistics_db, level_key)
	...
	if backend_settings.quests_enabled then
		...
		for contract_id, contract in pairs(active_contracts) do
			self:_commit_contract_progress(contract_id, contract, params)
...
QuestManager.update = function (self, statistics_db, dt, t)
	...
	self.available_contracts = quest_interface:get_available_contracts()
	self.mutators = quest_interface:get_mutators()
	self.active_mutators = self:_update_active_mutators(self.active_mutators)
	...
	for contract_id, contract in pairs(active_contracts) do
		repeat
			if not self:_progress_valid(contract, params) then
				local contract_session_progress = session_progress[contract_id]
				...
				local session_amount, negative_progress = self:_calculate_contract_session_progress(contract, params, false)
...
QuestManager._calculate_contract_session_progress = function (self, contract, params, finished_level)
	...
	if task then
		...
		elseif task_type == "ogre" then
			...
			local player_killed = statistics_db:get_stat(stats_id, "kills_per_breed", "skaven_rat_ogre")
			local player_assists = statistics_db:get_stat(stats_id, "kill_assists_per_breed", "skaven_rat_ogre")

			return player_killed + player_assists
		elseif task_type == "tome" then
			local mission_data = mission_system:get_level_end_mission_data("tome_bonus_mission")
			local num_collected = (mission_data and mission_data:get_current_amount()) or 0

			return num_collected
		elseif task_type == "grimoire" then
			local mission_data = mission_system:get_level_end_mission_data("grimoire_hidden_mission")
			local num_collected = (mission_data and mission_data:get_current_amount()) or 0
		...
		elseif task_type == "grenade_kills" then
			local local_player = Managers.player:local_player()
			local stats_id = local_player:stats_id()
			local player_killed = statistics_db:get_stat(stats_id, "kills_grenade")
			local player_assists = statistics_db:get_stat(stats_id, "kill_assists_grenade")
			local grenade_kills = player_killed + player_assists

			return grenade_kills
		elseif task_type == "open_chests" then
			return statistics_db:get_stat("session", "chests_opened")
		elseif task_type == "damage_taken_individual" then
			...
			local progress = statistics_db:get_stat("session", "level_progress_distance")
			...
			elseif player_dead or not last_checkpoint_progress or damage_taken > last_checkpoint_damage_taken + allowed_damage then
				if last_checkpoint_progress and damage_taken > last_checkpoint_damage_taken + allowed_damage and progress - last_checkpoint_progress > 5 then
		...
		elseif task_type == "avoid_deaths_team" then
			...
			local deaths = statistics_db:get_stat("session", "deaths")

			return math.max(0, starting_points - deaths)
		elseif task_type == "avoid_specials_damage_team" then
			local allowed_damage = 150
			local damage = 0
			local progress_percent = statistics_db:get_stat("session", "level_progress")

			for _, breed_name in ipairs(SPECIALS) do
				damage = damage + statistics_db:get_stat("session", "damage_taken_per_breed", breed_name)
			end
		...
		elseif task_type == "last_stand_waves_defeated" then
			...
			for i = 1, num_diffs, 1 do
				...
				if found or diff_name == difficulty then
					found = true
					waves = waves + statistics_db:get_stat(stats_id, "last_stand_waves_completed", diff_name)
				end
			end
		...
		elseif task_type == "find_event_items" then
			local local_player = Managers.player:local_player()
			local stats_id = local_player:stats_id()
			local event_items_found = statistics_db:get_stat(stats_id, "event_items_found")
...
QuestManager._commit_contract_progress = function (self, contract_id, contract, params)
	...
	local task_amount = self:_calculate_contract_session_progress(contract, params, true)
...
QuestManager.get_contract_progress = function (self, contract_id)
	local contract = self:get_contract_by_id(contract_id)
	local task = contract.requirements.task
...
QuestManager.has_contract_progressed = function (self, contract_id)
	local progress = self:get_contract_progress(contract_id)
...
QuestManager.get_contract_by_id = function (self, contract_id)
	local quest_interface = self.quest_interface
	local contracts = quest_interface:get_contracts()
...
QuestManager.accept_contract_by_id = function (self, contract_id)
	...
	quest_interface:set_contract_active(contract_id, true)
end

QuestManager.decline_contract_by_id = function (self, contract_id)
	local quest_interface = self.quest_interface

	quest_interface:set_contract_active(contract_id, false)
end

QuestManager.complete_contract_by_id = function (self, contract_id)
	local quest_interface = self.quest_interface

	quest_interface:complete_contract(contract_id)
end
...
QuestManager.should_contract_progress_be_shown = function (self, contract_id, ignore_level_and_difficulty)
	...
	local progress_valid = self:_progress_valid(contract, params, ignore_level_and_difficulty)
...
QuestManager.are_contracts_dirty = function (self)
	local quest_interface = self.quest_interface
	local dirty = quest_interface:are_contracts_dirty()
...
QuestManager.get_description_for_contract_id = function (self, contract_id)
	...
	local start_seed = self:get_random_seed_from_id(contract_id)
...
QuestManager.has_quest_progressed = function (self, contract_id)
	local progress = self:get_quest_progress(contract_id)
...
QuestManager.get_quest_by_id = function (self, quest_id)
	local quest_interface = self.quest_interface
	local quests = quest_interface:get_quests()
...
QuestManager.decline_quest_by_id = function (self, quest_id)
	...
	quest_interface:set_active_quest(quest_id, false)
end

QuestManager.complete_quest_by_id = function (self, quest_id)
	local quest_interface = self.quest_interface

	quest_interface:complete_quest(quest_id)
end
...
QuestManager.are_quests_dirty = function (self)
	local quest_interface = self.quest_interface
	local dirty = quest_interface:are_quests_dirty()
...
QuestManager.get_title_for_quest_id = function (self, quest_id)
	local quest = self:get_quest_by_id(quest_id)
	local start_seed = self:get_random_seed_from_id(quest_id)
...
QuestManager._update_expire_times = function (self, dt, t)
	...
	if quest_interface:are_expire_times_dirty() then
		self.expire_times = quest_interface:get_expire_times()
	...
	if self.expire_times then
		local quests_expired = self:has_time_expired(self.expire_times.quests_ttl)
		local contracts_expired = self:has_time_expired(self.expire_times.contracts_ttl)
		self.quests_expired = quests_expired
		self.contracts_expired = contracts_expired

		if quests_expired or contracts_expired then
			self:query_quests_and_contracts()
...
QuestManager.are_status_dirty = function (self)
	local quest_interface = self.quest_interface
	local dirty = quest_interface:are_status_dirty()
...
QuestManager.query_quests_and_contracts = function (self)
	...
	quest_interface:query_quests_and_contracts()
...
QuestManager._poll_backend = function (self, dt, t)
	local dirty_quests = self:are_quests_dirty()
	local dirty_contracts = self:are_contracts_dirty()
	...
	if self._query_quests_and_contracts then
		...
		self:_update_expire_times(dt, t)
	end
...
QuestManager.poll_reward = function (self)
	local quest_interface = self.quest_interface
	local reward = quest_interface:poll_reward()
