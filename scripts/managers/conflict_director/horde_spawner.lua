...
HordeSpawner.init = function (self, world, cover_points_broadphase)
	...
	self.world = world
	CurrentHordeSettings.ambush.max_size = self.max_composition_size(self, CurrentHordeSettings.ambush.composition_type)
	CurrentHordeSettings.vector.max_size = self.max_composition_size(self, CurrentHordeSettings.vector.composition_type)
...
HordeSpawner.execute_event_horde = function (self, t, terror_event_id, composition_type, limit_spawners, silent, group_template)
...
HordeSpawner.execute_ambush_horde = function (self, fallback, override_epicenter_pos)
	...
	self.hidden_cover_points(self, self.broadphase, epicenter_pos, player_and_bot_positions, found_cover_points, hidden_min_dist, hidden_max_dist)
	self.reset_sectors(self, hidden_sectors)
	self.calc_sectors(self, epicenter_pos, found_cover_points, hidden_sectors)

	if script_data.debug_hordes then
		self.render_sectors(self, hidden_sectors)
	end
	...
	local num_to_spawn = self.compose_horde_spawn_list(self, composition_type)
	...
	while last_spawner_count ~= spawner_count and spawner_count < num_to_spawn do
		...
		for i = 1, num_sectors, 1 do
			...
			if spawner then
				local breed_name = self.pop_random_horde_breed_only(self)
	...
	if spawner_count < num_to_spawn then
		...
		while 0 < missing do
			...
			if 0 < num_horde_spawns then
				breed_name = self.pop_random_horde_breed_only(self)
...
HordeSpawner.find_vector_horde_spawners = function (self, epicenter_pos, main_target_pos)
	...
	table.clear_array(found_cover_points, #found_cover_points)
	self.hidden_cover_points(self, self.broadphase, epicenter_pos, {
		epicenter_pos
...
HordeSpawner.execute_vector_horde = function (self, fallback)
	...
	local num_to_spawn, num_horde_breed, num_hidden_breed = self.compose_horde_spawn_list(self, composition_type)
	...
	if horde.horde_type == "vector" then
		self.play_sound(self, "enemy_horde_stinger", horde.epicenter_pos:unbox())
	end
...
HordeSpawner.update_event_horde = function (self, horde, t)
	if not horde.started then
		if horde.start_time < t then
			local success, amount = self.spawner_system:spawn_horde_from_terror_event_id(horde.terror_event_id, horde.composition_type, horde.limit_spawners, horde.group_template)
	...
	if hidden_spawns then
		for j = 1, #hidden_spawns, 1 do
			...
			if 0 < hidden_spawn.num_to_spawn and hidden_spawn.next_spawn_time < t then
				...
				hidden_spawn.next_spawn_time = t + 1
			end
		end
	end
