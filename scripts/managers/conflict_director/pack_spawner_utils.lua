...
PackSpawnerUtils.get_pack_by_size = function (size)
	local index = 1
	local pack_list = BreedPacksBySize[size]

	if not pack_list then
	end

	local current_spawn_weight = math.random(0, pack_list[index].spawn_weight)

	for i, pack in ipairs(pack_list) do
		local new_spawn_weight = math.random(0, pack.spawn_weight)
