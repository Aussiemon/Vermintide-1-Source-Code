...
MatchmakingManager.init = function (self, params)
	...
	self:add_default_filter_requirements()
...
MatchmakingManager.update = function (self, dt, t)
	...
	if self.is_server and lobby:is_joined() then
		if self.profile_update_time < t or self.matchmaking_ui.is_in_inn then
			...
			for i = 1, num_profiles, 1 do
				...
				local player_slot = lobby:lobby_data(index_name)
				local profile_owner = profile_synchronizer:owner(i)
	...
	if members then
		...
		for peer_id, _ in pairs(members) do
			local portrait_index = self:get_portrait_index(peer_id)
...
MatchmakingManager.update_cancel_input = function (self, dt)
	local is_matchmaking = self:is_game_matchmaking() and self.matchmaking_ui.is_in_inn
	...
	if is_matchmaking then
		if self.controller_cooldown and self.controller_cooldown > 0 then
			...
			if self.is_server and not enter_game and input_service and input_service:get("cancel_matchmaking", true) then
...
MatchmakingManager.get_random_level = function (self, level_area, game_mode, difficulty_rank, statistics_db, player_stats_id, optional_level_filter)
	if game_mode == "adventure" then
		return self:get_random_adventure_level(level_area, difficulty_rank, statistics_db, player_stats_id, optional_level_filter)
	elseif game_mode == "survival" then
		return self:get_random_survival_level(level_area, difficulty_rank, statistics_db, player_stats_id, optional_level_filter)
	end
end
...
MatchmakingManager.get_random_adventure_level = function (self, level_area, difficulty_rank, statistics_db, player_stats_id, optional_level_list)
	...
	for i = 1, #levels, 1 do
		...
		if game_mode == "adventure" then
			...
			if approved_location and self:level_unlocked_at_difficulty(random_level_key, difficulty_rank, statistics_db, player_stats_id) and not dlc_stat_dependency_func then
...
MatchmakingManager.find_game = function (self, level_key, difficulty, private_game, quick_game, game_mode, area, t, level_filter)
	if self.is_server then
		...
		local player_manager = Managers.player
		local number_of_players = player_manager:num_human_players()
		...
		if quick_game or level_filter then
			...
			level_key = self:get_random_level(area, game_mode, difficulty_rank, statistics_db, player_stats_id, level_filter)
		...
		local start_directly = num_connections > 0 or MatchmakingSettings.host_games == "always"
		local next_state = nil

		if start_directly or private_game or number_of_players > 1 or ALWAYS_HOST_GAME then
	...
	if is_matchmaking then
		if self.is_server then
			...
			if MatchmakingSettings.restart_search_after_host_cancel and game_search_data then
				...
				self:find_game(game_search_data.level_key, game_search_data.difficulty, false, game_search_data.quick_game, game_search_data.game_mode, game_search_data.area, t)
			else
...
MatchmakingManager.profiles_data = function (self)
	...
	for i = 1, num_profiles, 1 do
		local index_name = player_slots[i]
		local lobby_profile_data = lobby:lobby_data(index_name) or "available"
...
MatchmakingManager.rpc_matchmaking_request_profile = function (self, sender, client_cookie, host_cookie, profile)
	...
	local player_slot_available = lobby:lobby_data(player_slot) == "available"
...
MatchmakingManager.lobby_match = function (self, lobby_data, level_key, difficulty, game_mode, wanted_profile_id, player_peer_id)
	...
	if level_key then
		...
		if lobby_level then
			...
			local player_stats_id = player:stats_id()
			local statistics_db = self.statistics_db
			local level_approved = self:level_unlocked_at_difficulty(lobby_level, difficulty_rank, statistics_db, player_stats_id)
	...
	if wanted_profile_id then
		local hero_available = self:hero_available_in_lobby_data(wanted_profile_id, lobby_data)
		...
		for i = 1, 5, 1 do
			if MatchmakingSettings.hero_search_filter[i] == true then
				local hero_available = self:hero_available_in_lobby_data(i, lobby_data)
...
MatchmakingManager.request_join_lobby = function (self, lobby, state_context_params)
	...
	matchmaking_ui:set_search_zone_host_title("matchmaking_status_client")
	matchmaking_ui:large_window_set_title("status_matchmaking")
	matchmaking_ui:large_window_set_level(lobby.selected_level_key)
	matchmaking_ui:large_window_set_difficulty(lobby.difficulty)
