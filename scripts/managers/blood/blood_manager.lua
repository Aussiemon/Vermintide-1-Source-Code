...
BloodManager.spawn_blood_ball = function (self, position, direction, damage_type, hit_unit)
	if BloodSettings.blood_decals.enabled then
		if BloodSettings.blood_decals.num_decals > 0 then
			...
			local velocity = BloodSettings.blood_ball.damage_type_velocities[damage_type] or BloodSettings.blood_ball.damage_type_velocities.default

			Actor.add_velocity(actor, Vector3.normalize(direction) * velocity)
		end
