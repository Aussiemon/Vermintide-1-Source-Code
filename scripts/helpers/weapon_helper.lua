...
local hit_delta_squared = 1
WeaponHelper.test_angled_trajectory = function (physics_world, p1, p2, gravity, projectile_speed, angle, segment_list, sections)
	...
	if angle and projectile_speed then
		...
		for i = 1, sections, 1 do
			...
			local result, hit_position, _, _, actor = PhysicsWorld.immediate_raycast(physics_world, segment_pos1, current_velocity, Vector3.length(current_velocity), "closest", "collision_filter", "filter_ai_mover")
