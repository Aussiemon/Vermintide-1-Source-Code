...
Rewards.get_mission_results = function (self, completed_level, game_mode_key)
	...
	if game_mode_key == "survival" then
		self._add_missions_from_mission_system(self, mission_rewards, 1, 1)
	elseif completed_level then
		self._add_missions_from_mission_system(self, mission_rewards, difficulty_rank, level_length_modifier)
...
Rewards.get_level_end = function (self, completed_level, game_mode_key)
	local mission_rewards = self.get_mission_results(self, completed_level, game_mode_key)
	local gained_xp = 0
...
Rewards.award_experience = function (self, completed_level, game_mode_key)
	local level_start, start_experience = self.get_level_start(self)
	local level_end, end_experience = self.get_level_end(self, completed_level, game_mode_key)
	...
	if not completed_level or game_mode_key == "survival" then
		local token_rewards = self._loss_tokens(self)
		self._token_rewards = token_rewards

		self._query_upgrades_failed_game(self, level_start, level_end)
