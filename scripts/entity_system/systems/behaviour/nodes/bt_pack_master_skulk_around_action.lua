...
BTPackMasterSkulkAroundAction.run = function (self, unit, blackboard, t, dt)
	...
	if blackboard.skulk_time < t and blackboard.attack_cooldown < t then
		...
		local target_slot_extension = ScriptUnit.extension(blackboard.target_unit, "ai_slot_system")
		local dogpile = target_slot_extension.slots_count
...
BTPackMasterSkulkAroundAction.get_new_goal = function (self, unit, blackboard)
	if Unit.alive(blackboard.target_unit) then
		...
		pos = LocomotionUtils.outside_goal(blackboard.nav_world, unit, blackboard, min_dist, max_dist, angle, 5, above_below, above_below)
