...
BTRatlingGunnerShootAction.leave = function (self, unit, blackboard, t)
...
BTRatlingGunnerShootAction._update_target = function (self, unit, blackboard, action, data, t, dt)
	...
	if data.target_check < t then
		...
		local target, node_name, old_target_visible, old_target_visible_node_name = PerceptionUtils.pick_ratling_gun_target(unit, blackboard, old_target)
		...
		if target and target ~= old_target then
			...
			if Vector3.distance_squared(target_pos, self_pos) < target_switch_distance_sq and target_switch_distance_sq < Vector3.distance_squared(old_target_pos, self_pos) and VIEW_CONE < Vector3.dot(Vector3.normalize(data.shoot_direction_box:unbox()), Vector3.normalize(target_pos - self_pos)) then
...
BTRatlingGunnerShootAction._shoot = function (self, unit, blackboard)
	...
	local attack_template_damage_type = action.attack_template_damage_type[difficulty_rank]
	local action_data = {
		attack_template = action.attack_template,
		attack_template_damage_type = attack_template_damage_type,
		max_penetrations = action.max_penetrations,
