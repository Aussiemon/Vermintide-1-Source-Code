...
BTSkulkApproachAction.leave = function (self, unit, blackboard, t, reason)
...
BTSkulkApproachAction.get_random_goal_on_circle = function (self, unit, blackboard)
	...
	local drawer_stay = Managers.state.debug:drawer({
		mode = "retained",
	...
	for i = 1, TRIES, 1 do
		...
		if on_navmesh then
			...
			if script_data.ai_globadier_behavior then
				drawer_stay.sphere(drawer_stay, position + offset, 0.25, Colors.get("aqua_marine"))
			end
		...
		elseif script_data.ai_globadier_behavior then
			drawer_stay.sphere(drawer_stay, position + offset, 0.25, Colors.get_color_with_alpha("aqua_marine", 100))
		end
	end
