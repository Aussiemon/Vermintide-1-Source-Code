...
BTThrowSimpleProjectileAction.enter = function (self, unit, blackboard, t)
	...
	network_manager.anim_event(network_manager, unit, throw_attack.attack_anim)
	...
	local aim_pos = self._aim_to_position(self, self._aim_from_position(self, unit), target_unit, throw_attack.projectile_speed)
...
BTThrowSimpleProjectileAction._throw = function (self, unit, blackboard, action, throw_data, from_position, aim_rotation)
	...
	for i = 1, throw_attack.num_projectiles, 1 do
		...
		projectile_system.create_light_weight_projectile(projectile_system, damage_source, unit, from_position, direction, throw_attack.projectile_speed, throw_attack.projectile_max_range, action.collision_filter, impact_data, action.light_weight_projectile_particle_effect)
	end
...
BTThrowSimpleProjectileAction.run = function (self, unit, blackboard, t, dt)
	...
	local from_position = self._aim_from_position(self, unit)
	local to_position = self._aim_to_position(self, from_position, data.target_unit, data.attack.projectile_speed, data.aim_position:unbox())
	...
	locomotion_extension.set_wanted_rotation(locomotion_extension, flat_aim_rotation)

	if not data.thrown and data.throw_time < t then
		self._throw(self, unit, blackboard, action, data, from_position, aim_rotation)
	end
