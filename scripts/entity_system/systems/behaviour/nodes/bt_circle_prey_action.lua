...
BTCirclePreyAction.enter = function (self, unit, blackboard, t)
	...
	elseif self._tree_node.action_data.despawn_on_outside_navmesh then
		AiUtils.kill_unit(unit, unit)
	else
...
BTCirclePreyAction.run = function (self, unit, blackboard, t, dt)
	...
	if not blackboard.ninja_vanish and (nav_ext.has_reached_destination(nav_ext) or (not nav_ext.is_following_path(nav_ext) and not nav_ext.is_pathing(nav_ext))) then
		local goal_position = self.get_new_goal(self, unit, blackboard)
