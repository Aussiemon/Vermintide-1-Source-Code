...
BTJumpSlamImpactAction.jump_slam_impact = function (self, unit, blackboard, t)
	...
	BTJumpSlamImpactAction.impact_damage(unit, t, action.stagger_radius, action.stagger_distance, action.stagger_impact, action.damage, action.damage_type, action.max_damage_radius, impact_position)
				...
				if damage then
					...
					if 0 < raw_damage then
						...
						if is_inside_radius then
							...
							DamageUtils.add_damage_network(ai_unit, attacking_unit, damage_done, "full", damage_type, Vector3(0, 0, -1))
						end
					end
				end
			end
