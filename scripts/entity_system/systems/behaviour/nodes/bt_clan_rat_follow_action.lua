...
BTClanRatFollowAction.enter = function (self, unit, blackboard, t)
	...
	if self:_should_walk(destination, rat_pos, ENTER_WALK_DISTANCE_SQ, rot) then
...
BTClanRatFollowAction._update_walking = function (self, unit, blackboard, dt, t)
	...
	if not self:_should_walk(blackboard.navigation_extension:destination(), self_pos, LEAVE_WALK_DISTANCE_SQ, rot) or blackboard.walk_timer < t or (velocity_away and WALK_MAX_TARGET_VELOCITY < velocity_away) then
...
BTClanRatFollowAction.follow = function (self, unit, blackboard, t, dt)
	...
	elseif (current_speed > breed.run_speed + 0.1 or blackboard.deacceleration_factor) and target_distance < 2 * weapon_reach + CHASE_DEACCELERATION_DISTANCE then
		...
		if sign > 0 and current_speed < breed.run_speed and target_distance > weapon_reach + 0.5 and not breed.run_speed then
		end
