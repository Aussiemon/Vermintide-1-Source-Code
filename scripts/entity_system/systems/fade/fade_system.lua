...
FadeSystem.update = function (self, context, t)
	...
	for i = 1, num_fade_units, 1 do
		...
		local value = self.calculate_and_set_jitter_alpha(self, camera_position_flat, camera_position_height, fade_unit)
...
FadeSystem.update_ai_units = function (self, camera_position, camera_position_flat, camera_position_height)
	...
	for i = 1, num_ai_units, 1 do
		...
		if extension then
			local cached_value = extension.cached_value
			local value = self.calculate_and_set_jitter_alpha(self, camera_position_flat, camera_position_height, unit, cached_value)
			extension.cached_value = value
...
FadeSystem.calculate_and_set_jitter_alpha = function (self, camera_position_flat, camera_height, unit, cached_value)
	local fade_unit_position = unit_world_position(unit, unit_node(unit, "j_hips"))
	...
	if height_difference < MAX_HEIGHT_DIFFERENCE then
		value = math.min((to_vec_length - MIN_FADE_DISTANCE)/CHANGE_DISTANCE, 1) - 1
	else
		value = 0
	end
