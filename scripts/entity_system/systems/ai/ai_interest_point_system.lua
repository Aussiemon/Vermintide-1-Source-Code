...
AIInterestPointSystem.spawn_interest_points = function (self)
	...
	while num_to_spawn > 0 and interest_point_unit ~= nil do
		...
		local packs_by_size = BreedPacksBySize[num_valid_to_spawn]
		...
		for i = 1, point_extension.points_n, 1 do
			...
			if point.is_position_on_navmesh then
				...
				local id = conflict:spawn_queued_unit(breed, Vector3Box(Vector3Aux.unbox(point.position)), point.rotation, spawn_category, spawn_animation, spawn_type, inventory_template, group_data, point)
				point[1] = id
			else
