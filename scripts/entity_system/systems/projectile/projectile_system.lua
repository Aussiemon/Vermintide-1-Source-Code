...
require("scripts/unit_extensions/weapons/projectiles/timed_projectile_extension")
...
ProjectileSystem.on_add_extension = function (self, world, unit, extension_name, ...)
	if extension_name == "PlayerProjectileUnitExtension" or extension_name == "PlayerProjectileHuskExtension" then
		self.statistics_db:register(unit, "weapon")
	end
...
ProjectileSystem.on_remove_extension = function (self, unit, extension_name)
	if extension_name == "PlayerProjectileUnitExtension" or extension_name == "PlayerProjectileHuskExtension" then
		...
		if projectile_extension.projectile_info.is_grenade then
			local owner_unit = projectile_extension.owner_unit
			local kills = self.statistics_db:get_stat(unit, "kills_total")
			local owner_player = Managers.player:owner(owner_unit)

			if owner_player then
				local stats_id = owner_player:stats_id()
				local old_multikill = self.statistics_db:get_stat(stats_id, "best_projectile_multikill")

				if old_multikill < kills then
					local multikill_diff = kills - old_multikill

					self.statistics_db:modify_stat_by_amount(stats_id, "best_projectile_multikill", multikill_diff)
				end
			end
		end

		self.statistics_db:unregister(unit)
	end
...
ProjectileSystem.rpc_spawn_player_projectile = function (self, sender, owner_unit_id, position, rotation, scale, angle, target_vector, speed, item_name_id, item_template_name_id, action_name_id, sub_action_name_id, gaze_settings)
	...
	self:spawn_player_projectile(owner_unit, position, rotation, scale, angle, target_vector, speed, item_name, item_template_name, action_name, sub_action_name, 0, gaze_settings)
end
...
ProjectileSystem.rpc_spawn_pickup_projectile_limited = function (self, sender, projectile_unit_name_id, projectile_unit_template_name_id, network_position, network_rotation, network_velocity, network_angular_velocity, pickup_name_id, spawner_unit_id, limited_item_id, pickup_spawn_type_id)
	...
	if pickup_name == "grain_sack" then
		...
		mission_system:increment_goal_mission_counter("farm_dont_drop_sacks", 1, true)
	end
...
ProjectileSystem.rpc_spawn_true_flight_projectile = function (self, sender, owner_unit_id, target_unit_id, true_flight_template_id, position, rotation, angle, target_vector, speed, item_name_id, item_template_name_id, action_name_id, sub_action_name_id, scale)
	return
end

ProjectileSystem.spawn_true_flight_projectile = function (self, owner_unit, target_unit, true_flight_template_name, position, rotation, angle, target_vector, speed, item_name, item_template_name, action_name, sub_action_name, scale)
	local action = Weapons[item_template_name].actions[action_name][sub_action_name]
	local projectile_info = action.projectile_info
	local gravity_settings = projectile_info.gravity_settings
	local trajectory_template_name = projectile_info.trajectory_template_name
...
ProjectileSystem.physics_cb_light_weight_projectile_hit = function (self, projectile_data, hits)
	...
	local hit_data = DamageUtils.process_projectile_hit(self.world, projectile_data.damage_source, projectile_data.owner_unit, true, hits, impact_data, projectile_data.direction:unbox(), false)
