...
local function add_grimoire()
	...
	mission_system:update_mission(mission_name, true, nil, nil, nil, true)
...
local function remove_grimoire()
	...
	mission_system:update_mission(mission_name, false, nil, nil, nil, true)
...
InventorySystem.rpc_give_equipment = function (self, sender, game_object_id, slot_id, item_name_id, position)
	...
	if Unit.alive(unit) and not ScriptUnit.extension(unit, "status_system"):is_dead() then
		...
		if not owner.remote then
			...
			self.network_transmit:send_rpc("rpc_give_equipment", owner:network_id(), game_object_id, slot_id, item_name_id, position)
		end
	else
		failed = true
	end
...
InventorySystem.rpc_add_equipment = function (self, sender, go_id, slot_id, item_name_id)
	...
	inventory:add_equipment(slot_name, item_name)
end
