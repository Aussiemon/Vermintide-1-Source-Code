...
BackendTester.start = function (self, max_iterations, sleep_duration)
	...
	print(string.format("Base time %ss", self:_duration(sleep_duration * max_iterations)))
end
...
BackendTester.update = function (self, dt)
	...
	elseif self._state == "poll" then
		...
		if result then
			...
			local time_left = self:_time_left(percent)
			local bar = self:_progress_bar(percent)
